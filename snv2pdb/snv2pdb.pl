...
if ($#ARGV != 5) {die "Program used with parameters [pdb] [chid] [uniprotAC] [rare SNV] [common SNV] [SIFTS mapping file]\n";}
...
my @rare=open_file($ARGV[3]);
#read list of common variants
my @common=open_file($ARGV[4]);

#parse PDB swissport mapping
my @sifts=open_file($ARGV[5]);
my (%pdb2uni);  
