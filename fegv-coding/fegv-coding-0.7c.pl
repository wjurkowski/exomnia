...
my $DB="/home/wiktor/Szafa/db/uniprot_fasta/";
...
#output file
my $outfile=substr($ARGV[0],0,rindex($ARGV[0],"."))."-aaSNV_scores-".$ARGV[1].".txt";

open (GOUT, ">$outfile") or die "Can not create output file $outfile : $!";
...
for ($l=0;$l<=$#inputs;$l++){
 ...
 $nAA =~ s/\s+$//;
 $oAA =~ s/\s+$//;
 $alignment=$vpar[8];#in fasta format
 $tree=$vpar[9];
 my $waryjat="$oAA$resn$nAA";# gv e.g. G124A
 my @seq=open_file($fasta);
 my $zlepek="$identif\t$uniprotAC\t$uniprotID\t$waryjat";
 push(@gener,$zlepek);
 ...
 if($mSNAP == 1 or $mALL == 1){
	...
	if($uniprotAC eq $prot){
	  ...
	  unless($prot eq "blaszka"){
		...
		print "run SNAP------------------------------:\n";
		my $User = "snapc";
		my $Password="2f929b831d76b9aa4fe6b2d6a5b734af8c3ca99c";
		my $url = 'https://www.rostlab.org/services/snapc/snapcache.html';
		...
		#my $URL = "https://www.rostlab.org/services/snapc/snapcache.cgi";
